...
def OMCal(Spd, Acc, VSP):  # Unit here: Spd -- m/s, Acc -- m/s^2 and VSP -- kWatt/tonne
    ...
    # conversion factor for speed from m/s to mph and acceleration from m/s^2 to mph/s
    c1 = 2.23693629
    # vector of sample size for each of 23 operating mode
...
def movestar(veh_type, speed):
    ...
    df_vehicle_src_coeff = pd.read_csv(
        os.path.join(BASE_DIR, "VehicleSrcCoeff.csv"))
    ...
    VSP = A * speed + B * speed ** 2 + C * \
        speed ** 3 + M * np.multiply(Acc, speed)
    VSP = (
        VSP / f
    )  # Scaled Tractive Power: Unit: speed -- m/s; acceleration -- m/s^2 and VSP -- kWatt/tonne (all for the purpose of VSP calculation) No consideration of road grade

    # outputres["VSP"] = VSP.tolist()
    # Obtain the OpMode distribution
    size_bin = OMCal(
        speed, Acc, VSP
    )  # Unit: speed -- m/s, Acc -- m/s^2 and VSP -- kWatt/tonne
    ...
    outputcol = (
        "CO(g),HC(g),NOx(g),PM2.5_Ele(g),PM2.5_Org(g),Energy(KJ),CO2(g),Fuel(g),TT(s)"
    )
