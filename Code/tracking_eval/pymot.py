...
class MOTEvaluation:
    ...
    def resetMapping(self):
        ...
        self.hypo_map_ = (
            {}
        )  # save most recent groundtruth id for each hypothesis id. Only updates, no deletions of keys.
    ...
    def get_hypotheses_frame(self, timestamp):
        ...
        hypotheses_frames = list(filter(
            hypothesis_frame_chronologically_close, self.hypotheses_["frames"]
        ))
        ...
        if len(hypotheses_frames) > 1:
            ...
            print(
                "> 1 hypotheses timestamps found for timestamp %f with sync delta %f"
                % (
                    timestamp,
                    self.sync_delta_,
                )
        ...
        if len(hypotheses_frames) == 0:
            write_stderr_red("Warning:", "No hypothesis timestamp found for timestamp %f with sync delta %f" % (
    ...
    def evaluateFrame(self, frame):
        ...
        for gt_id in self.mappings_.keys():
            ...
            if overlap >= self.overlap_threshold_:
                # LOG.info(
                #     "Keeping correspondence between %s and %s"
        ...
        munkres_matrix = [
            [self.munkres_inf_ for i in range(len(hypotheses))]
            for j in range(len(groundtruths))
        ]  # TODO make square matrix

        # print(len(groundtruths)) 37
        # print(len(hypotheses))  47
        ...
        for i in range(len(groundtruths)):
            ...
            if groundtruth["id"] in correspondences.keys():
                # LOG.info("Groundtruth %s already in correspondence" %
                #          groundtruth["id"])
                continue
            ...
            for j in range(len(hypotheses)):
                ...
                if hypothesis["id"] in correspondences.values():
                    # LOG.info(
                    #     "Hypothesis %s already in correspondence" % hypothesis["id"]
                    # )
                    continue
        ...
        for gt_index, hypo_index in indices:
            ...
            if (
                munkres_matrix[gt_index][hypo_index] == self.munkres_inf_
            ...
            # LOG.info(
            #     "Correspondence found: %s and %s (overlap: %f)"
            ...
            # print('='*60)
            if (
                gt_id in self.gt_map_
                and self.gt_map_[gt_id] != hypo_id
                and not groundtruths[gt_index].get("dco", False)
            ):
                # LOG.info(
            ...
            if hypo_id in self.hypo_map_ and self.hypo_map_[hypo_id] != gt_id:
                ...
                old_gt_dco = list(filter(
                    lambda g: g["id"] == old_gt_id and g.get(
                        "dco", False), groundtruths
                ))

                assert len(old_gt_dco) <= 1
                if not (
                    groundtruths[gt_index].get(
                        "dco", False) and len(old_gt_dco) == 1
                ):
                    # LOG.info(
            ...
            for mapping_gt_id, mapping_hypo_id in list(self.mappings_.items()):
                ...
                gt_with_mapping_gt_id_dco = list(filter(
                    lambda g: g["id"] == mapping_gt_id and g.get("dco", False),
                    groundtruths,
                ))

                if len(gt_with_mapping_gt_id_dco) == 1 and groundtruths[gt_index].get(
                    "dco", False
                ):
                ...
                #                    print "DIFF DCO %s" % (gt_id), groundtruths[gt_index]
                    ...
                    if (mapping_gt_id == gt_id and mapping_hypo_id != hypo_id) or (
                        mapping_gt_id != gt_id and mapping_hypo_id == hypo_id
                    ):
                        # LOG.info(
                        ...
                        g = list(
                            filter(lambda g: g["id"] == gt_id, groundtruths))
                        h = list(
                            filter(lambda h: h["id"] == hypo_id, hypotheses))
            ...
            # if (
            #     self.recoverable_mismatches_ + self.non_recoverable_mismatches_
            #     != self.mismatches_
            # ):
            #     LOG.info(
        ...
        for groundtruth in groundtruths:
            ...
            if (
                groundtruth["id"] not in correspondences.keys()
        ...
        self.total_groundtruths_ += len(
            groundtruths
        )  # Number of objects (ground truths) in current frame
    ...
    def calcMOTA(abs_stats):
        ...
        return (
            ...
            - float(
                abs_stats["misses"]
                + abs_stats["false positives"]
                + abs_stats["mismatches"]
            )
            / num_gt
        )
    ...
    def calcRelativeStatistics(abs_stats):
        ...
        num_gt_tracks = (
            abs_stats["lonely ground truth tracks"]
            + abs_stats["covered ground truth tracks"]
        )
        ...
        return {
            ...
            "recoverable mismatch rate": float(abs_stats["recoverable mismatches"])
            / gt,
            "non-recoverable mismatch rate": float(
                abs_stats["non-recoverable mismatches"]
            )
            / gt,
            "track precision": float(abs_stats["covering hypothesis tracks"])
            / abs_stats["hypothesis tracks"]
            if abs_stats["hypothesis tracks"] != 0
            else 0.0,
            "track recall": float(abs_stats["covered ground truth tracks"])
            / num_gt_tracks
            if num_gt_tracks != 0
            else 0.0,
        }
    ...
    def getMOTA(self):
        ...
        # raise("No ground truth. MOTA calculation not possible")
            ...
            ) / float(self.total_groundtruths_)
        return mota
    ...
    def getRelativeStatistics(self):
        ...
        return {
            ...
            "non-recoverable mismatch rate": float(self.non_recoverable_mismatches_)
            / gt,
            "track precision": float(len(self.hypo_map_.keys()))
            / len(self.hypothesis_ids_)
            if len(self.hypothesis_ids_) != 0
            else 0.0,
            "track recall": float(len(self.gt_map_.keys())) / len(self.groundtruth_ids_)
            if len(self.groundtruth_ids_) != 0
            else 0.0,
        }
    ...
    def printResults(self):
        ...
        # print("Non recoverable mismatches  %d" %
        #       self.non_recoverable_mismatches_)
    ...
    def printLegacyFormat(self):
        ...
        print(
            "DIFF TOTALS: groundT %.0f\t miss %.0f\t falseP %.0f\t mismatch %0.f"
            % (
                self.total_groundtruths_,
                self.misses_,
                self.false_positives_,
                self.mismatches_,
            )
        )
        print(
            "ABS TOTALS: groundT %.0f\t corr %.0f\t failedCorr %.0f\t overlap_ratio %.2f\t miss %.0f\t falseP %.0f\t mismatch %0.f"
            % (
                self.total_groundtruths_,
                0,
                0,
...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--check_format",
                        action="store_true", default=True)
    parser.add_argument("-v", "--visual_debug_file")
    parser.add_argument("-i", "--iou", default=0.2,
                        type=float, help="iou threshold")
