...
def plot_one_box(x, im, color=(128, 128, 128), label=None, line_thickness=3, id=None):
    ...
    for i, img in enumerate(images):
        ...
        if len(targets) > 0:
            ...
            for j, box in enumerate(boxes.T):
                ...
                if cls in class_id_count.keys():
                    class_id_count[cls] += 1
                else:
                    class_id_count.update({cls: 0})
                id = class_id_count[cls]
                if labels or conf[j] > 0.25:  # 0.25 conf thresh
                    label = "%s" % cls if labels else "%s %.1f" % (cls, conf[j])
                    plot_one_box(
                        box, mosaic, label=label, color=color, line_thickness=tl, id=id
                    )
